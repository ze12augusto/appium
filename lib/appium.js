...
Appium.prototype.configure = function (args, desiredCaps, cb) {
  ...
  if( deviceType === "android" ){
    ...
    var AppiumController = this;
    AppiumController.launchAndroidEmulator( desiredCaps.platformVersion );
    console.log("preparing for verifyHasAlreadyConnectedAndroid");
    AppiumController.verifyHasAlreadyConnectedAndroid( function(){

      AppiumController.device = AppiumController.getNewDevice(deviceType);
      AppiumController.device.configure(args, desiredCaps, cb);
      AppiumController.device.onResetTimeout = function () { AppiumController.resetTimeout(); }.bind(AppiumController);
...
Appium.prototype.verifyHasAlreadyConnectedAndroid = function( callback ){

  exec('./verifyHasAlreadyConnectedAndroid.sh',
    function (error, stdout, stderr) {
      console.log('verifyHasAlreadyConnectedAndroid:stdout: ' , stdout);
      console.log('verifyHasAlreadyConnectedAndroid:stderr: ' , stderr);
      if (error !== null) {
        console.log('verifyHasAlreadyConnectedAndroid:exec error: ' , error);
      }
      callback();
  });
}

Appium.prototype.launchAndroidEmulator = function( platformVersion ){

  exec('./launchAndroidEmulatorToTest.sh ' + platformVersion,
    function (error, stdout, stderr) {
      console.log('launchAndroidEmulatorToTest:stdout: ' , stdout);
      console.log('launchAndroidEmulatorToTest:stderr: ' , stderr);
      if (error !== null) {
        console.log('launchAndroidEmulatorToTest:exec error: ' , error);
      }
  });
}
